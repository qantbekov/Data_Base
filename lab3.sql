...
SELECT building, course_num FROM (
         SELECT building, count(course_id) as course_num FROM section
         group by building) sub
WHERE course_num in (SELECT max(course_num)
    FROM (SELECT building, count(course_id) as course_num
         FROM section group by building) sub);

SELECT dept_name , course_num FROM (
         SELECT dept_name, count(course_id) as course_num FROM course
         group by dept_name) sub
WHERE course_num in (SELECT min(course_num)
    FROM (SELECT dept_name, count(course_id) as course_num
