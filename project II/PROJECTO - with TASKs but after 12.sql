...
--------------------------------------------------
--1task
SELECT * FROM customer inner join delivering_order on customer.customer_id = delivering_order.customer_id WHERE tracking_number = 123456;
INSERT INTO delivering_order VALUES (556, '2021-12-05','not yet',20,55,78,1,NULL);
--2task
SELECT customer.customer_id, count(orders.customer_id) FROM customer inner join orders on customer.customer_id = orders.customer_id group by customer.customer_id order by count(orders.customer_id) desc limit 1;
--3task
SELECT product_name, (product_price * product.sold_in_2021) as total_sum FROM product group by product_id, sold_in_2021 order by sold_in_2021 desc limit 2;
--4task---
SELECT product_name, sold_in_2021 as number FROM product group by product_id,sold_in_2021 order by sold_in_2021 desc limit 2;
--7task
...
CREATE trigger a
    ...
    for each row execute procedure bill();
